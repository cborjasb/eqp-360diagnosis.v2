/**
 * Dimensiones BASE de perdurabilidad empresarial.
 *
 * Son las 7 dimensiones universales del modelo "Empresas que Perduran"
 * (las mismas que existían antes en lib/constants.ts). Aplican a cualquier
 * empresa y sirven como núcleo común; cada sector las puede usar tal cual,
 * reescribir el texto de sus preguntas, o sumar dimensiones propias.
 */

import { DiagDimension } from '../questionnaire-types';

export const BASE_DIMENSIONS: DiagDimension[] = [
  {
    key: 'finanzas',
    label: 'Finanzas',
    iconName: 'Banknote',
...
/** Acceso por key, por si un sector quiere reutilizar una dimensión puntual. */
export const BASE_DIMENSION_BY_KEY: Record<string, DiagDimension> = Object.fromEntries(
  BASE_DIMENSIONS.map((d) => [d.key, d]),
);
